# Execution Plan v2 — Bumrah vs Steyn: Different Engines, Same Job

Revised after Codex review of v1. All tool-fit issues addressed.
...
**Actions:**
1. Run `extract_angles.py` on the Bumrah clip. Check landmark confidence on the hero frames (gather, FFC, release).
2. Run on the selected Steyn clip. Same check.
3. For each hero frame, verify that ALL four X-Factor joints (L/R shoulder, L/R hip) have confidence > 0.5.
4. Verify that front knee joints (hip, knee, ankle) have confidence > 0.4.

**Decision gate:**
- All hero frames pass → proceed to Phase 3.
- Bumrah passes, Steyn fails → try alternative Steyn clip. If all fail → simplify overlay (only draw what has high confidence).
- Both fail on key frames → re-extract at finer frame interval, or choose different hero frames.
...
3. Render a single test side-by-side card. Confirm layout, divider, scale matching.
4. Compose a 10-second stub video using placeholder frames. Confirm dissolves, text, Ken Burns, and encoding all work.

**Decision gate:**
- All four checks pass → proceed to Phase 4.
- Any check fails → fix before proceeding. Do not start the full render with broken tooling.
...
### Step 4.2: Render skeleton overlays on hero frames

Using the adapted `overlay_renderer.py` in `mode="comparison"`:
...
### Step 4.3: Apply colour grading

1. Run `_apply_colour_grade()` on all footage frames.
